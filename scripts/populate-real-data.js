...
async function populateRealData() {
  console.log("Starting to populate real TikTok data...\n");
  ...
  for (const [tagName, videos] of Object.entries(hashtagGroups)) {
    ...
    const trendData = {
      ...
      title: `#${tagName.charAt(0).toUpperCase() + tagName.slice(1)} Trending`,
      description: `Real trending content with #${tagName}. ${videos.length} videos analyzed from actual TikTok data.`,
      ...
      aiPrediction: `Real trend with ${videos.length} videos. Engagement rate ${avgEngagement.toFixed(1)}%. Source: TikTok API.`,
    };
    ...
    try {
      ...
      stats.trends++;
      console.log(`✓ Created trend: ${trend.title} (${trend.views} views)`);
      ...
      console.error(`✗ Error creating trend ${tagName}:`, error.message);
    }
  }
  ...
  for (const [username, creatorData] of Object.entries(uniqueCreators)) {
    ...
    try {
      ...
      stats.creators++;
      console.log(`✓ Created creator: ${creatorData.nickName || username} (${creatorData.fans || 0} followers)`);
    } catch (error) {
      console.error(`✗ Error creating creator ${username}:`, error.message);
    }
  }
  ...
  for (const [tagName, videos] of Object.entries(hashtagGroups)) {
    ...
    try {
      ...
      stats.hashtags++;
      console.log(`✓ Created hashtag: #${tagName} (${formatViews(totalViews)} views)`);
    } catch (error) {
      console.error(`✗ Error creating hashtag ${tagName}:`, error.message);
    }
  }
  ...
  for (const [musicId, soundData] of Object.entries(uniqueSounds)) {
    ...
    try {
      ...
      stats.sounds++;
      console.log(`✓ Created sound: ${soundData.musicName || "Unknown"} (${totalUses} uses)`);
    } catch (error) {
      console.error(`✗ Error creating sound ${musicId}:`, error.message);
    }
  }
